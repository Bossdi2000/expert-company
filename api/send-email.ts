...
// Fallback to anon key if service key isn't provided
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const adminEmail = process.env.VITE_ADMIN_NOTIFY_EMAIL || process.env.ADMIN_NOTIFY_EMAIL || undefined;
    
    const host = req.headers.host || 'expertinvest.xyz';
    ...
    return res.status(hasError ? 500 : 200).json({ ok: !hasError, sent: sends.length });
  } catch (err: any) {
